...
class CacheWarmingService {
  ...
  private initializeWarmingTasks(): void {
    ...
    this.addTask(
      ...
      async () => {
        ...
        const totalWeeklyPnl = customers.reduce(
          (sum: number, c: any) => sum + (c.weekly_pnl || 0),
          0
        );
      ...
      30000,
      'Customer statistics for dashboard overview'
    );
    ...
    this.addTask(
      ...
      async () => {
        ...
        const mainGroup = groups.main_group || {
          chat_id: '-2714719687',
      ...
      30000,
      'Group members data for management interface'
    );
  ...
  private async loadCustomers(): Promise<any[]> {
    try {
      ...
      if (
        ...
        process.env.CACHE_VERBOSE === 'true'
      ) {
        console.log(
          `📋 Loaded ${customers.length} customers for cache warming`
        );
      }
      ...
      if (!isTestEnv) {
        console.warn(
          '⚠️ Cache warming: customers.json not found, using empty array'
        );
      }
  ...
  async warmCache(
    ...
    if (verbose) {
      console.log(
        `🔥 Starting cache warming for priorities: ${priorities.join(', ')}`
      );
    }

    // Filter tasks by priority
    const tasksToRun = this.warmingTasks.filter(task =>
    ...
    for (const task of tasksToRun) {
      try {
        if (verbose) {
          console.log(
            `  ⏳ Warming ${task.key} (${task.priority}): ${task.description}`
          );
        }
    ...
    if (verbose) {
      console.log(
        `🔥 Cache warming completed: ${completedTasks}/${tasksToRun.length} tasks in ${totalTime}ms`
      );
      if (errorCount > 0) {
  ...
  async isCacheWarm(): Promise<boolean> {
    const criticalTasks = this.warmingTasks.filter(
