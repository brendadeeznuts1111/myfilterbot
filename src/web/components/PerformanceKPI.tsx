...
const getRiskLevel = (volatility: number, maxDrawdown: number): 'low' | 'medium' | 'high' | 'extreme' => {
...
export function PerformanceKPI({
  ...
  const { data: metricsData, error, loading, lastUpdated } = useAPI<{
    analytics: {
  ...
  const advancedMetrics = useMemo(() => {
    ...
    const { balance_trend, performance_metrics, transaction_summary, activity_stats } = metricsData.analytics;
    ...
    const totalVolume = performance_metrics.total_deposits + Math.abs(performance_metrics.total_withdrawals);
    const winRate = transaction_summary.by_type.deposits / Math.max(1, transaction_summary.total) * 100;
    ...
    const recoveryFactor = maxDrawdown === 0 ? 999 : Math.abs(performance_metrics.net_pnl) / maxDrawdown;
    
    // Sharpe ratio calculation
    const returns = [];
    for (let i = 1; i < balance_trend.length; i++) {
      if (balance_trend[i-1] !== 0) {
        returns.push((balance_trend[i] - balance_trend[i-1]) / balance_trend[i-1]);
      }
    }
  ...
  useEffect(() => {
    ...
    if (advancedMetrics.riskLevel === 'extreme') alerts.push('Extreme risk level');
  ...
  const generateAdvancedKPIs = (): KPIData[] => {
    ...
    const { performance_metrics, transaction_summary, activity_stats } = metricsData.analytics;
    ...
    const kpis: KPIData[] = [
      // Core Performance Metrics
      {
        ...
        status: performance_metrics.net_pnl > 0 ? 'excellent' : 
                performance_metrics.net_pnl > -1000 ? 'warning' : 'critical',
        icon: '💰',
        priority: 1
      },
      {
        ...
        format: 'currency',
        timeframe: selectedTimeframe,
        status: performance_metrics.net_pnl > 1000 ? 'excellent' : 
                performance_metrics.net_pnl > 0 ? 'good' : 
                performance_metrics.net_pnl > -500 ? 'warning' : 'critical',
        icon: '📈',
        priority: 2
      },
      {
        ...
        timeframe: selectedTimeframe,
        target: 0.15,
        status: performance_metrics.roi_percentage > 15 ? 'excellent' :
                performance_metrics.roi_percentage > 5 ? 'good' :
                performance_metrics.roi_percentage > -5 ? 'warning' : 'critical',
        icon: '🎯',
        priority: 3
      },
      ...
      // Risk Management Metrics
      {
        ...
        trend: advancedMetrics.maxDrawdown < 10 ? 'up' : advancedMetrics.maxDrawdown < 25 ? 'neutral' : 'down',
        format: 'percentage',
        timeframe: selectedTimeframe,
        target: 0.10,
        status: advancedMetrics.maxDrawdown < 5 ? 'excellent' :
                advancedMetrics.maxDrawdown < 15 ? 'good' :
                advancedMetrics.maxDrawdown < 25 ? 'warning' : 'critical',
        icon: '⚠️',
        priority: 4
      },
      {
        ...
        trend: advancedMetrics.volatility < 10 ? 'up' : advancedMetrics.volatility < 20 ? 'neutral' : 'down',
        format: 'percentage',
        timeframe: selectedTimeframe,
        target: 0.15,
        status: advancedMetrics.volatility < 5 ? 'excellent' :
                advancedMetrics.volatility < 15 ? 'good' :
                advancedMetrics.volatility < 30 ? 'warning' : 'critical',
        icon: '📊',
        priority: 8
      },
      {
        ...
        trend: advancedMetrics.riskLevel === 'low' ? 'up' : 
               advancedMetrics.riskLevel === 'medium' ? 'neutral' : 'down',
        format: 'text',
        timeframe: selectedTimeframe,
        status: advancedMetrics.riskLevel === 'low' ? 'excellent' :
                advancedMetrics.riskLevel === 'medium' ? 'good' :
                advancedMetrics.riskLevel === 'high' ? 'warning' : 'critical',
        icon: '🛡️',
        priority: 5
      },
      ...
      // Trading Activity Metrics
      {
        ...
        trend: advancedMetrics.winRate > 60 ? 'up' : advancedMetrics.winRate > 40 ? 'neutral' : 'down',
        format: 'percentage',
        timeframe: selectedTimeframe,
        target: 0.60,
        status: advancedMetrics.winRate > 70 ? 'excellent' :
                advancedMetrics.winRate > 50 ? 'good' :
                advancedMetrics.winRate > 30 ? 'warning' : 'critical',
        icon: '🎲',
        priority: 6
      },
      {
        ...
        trend: advancedMetrics.profitFactor > 1.5 ? 'up' : advancedMetrics.profitFactor > 1 ? 'neutral' : 'down',
        format: 'ratio',
        timeframe: selectedTimeframe,
        target: 1.5,
        status: advancedMetrics.profitFactor > 2 ? 'excellent' :
                advancedMetrics.profitFactor > 1.2 ? 'good' :
                advancedMetrics.profitFactor > 1 ? 'warning' : 'critical',
        icon: '⚖️',
        priority: 7
      },
      {
        ...
        trend: advancedMetrics.sharpeRatio > 1 ? 'up' : advancedMetrics.sharpeRatio > 0 ? 'neutral' : 'down',
        format: 'ratio',
        timeframe: selectedTimeframe,
        target: 1.5,
        status: advancedMetrics.sharpeRatio > 2 ? 'excellent' :
                advancedMetrics.sharpeRatio > 1 ? 'good' :
                advancedMetrics.sharpeRatio > 0 ? 'warning' : 'critical',
        icon: '📐',
        priority: 9
      },
      ...
      },
      {
        ...
        format: 'number',
        timeframe: selectedTimeframe,
        status: transaction_summary.total > 10 ? 'excellent' :
                transaction_summary.total > 5 ? 'good' : 'warning',
        icon: '📋',
        priority: 12
      }
    ];
    ...
    let sortedKpis = kpis.sort((a, b) => (a.priority || 999) - (b.priority || 999));
  ...
  const getAdvancedStatusStyle = (status: KPIData['status'], isDark: boolean, priority?: number) => {
    const baseStyles = {
      excellent: isDark 
    ...
    const priorityGlow = priority && priority <= 3 ? 'shadow-lg transform hover:scale-105' : 'shadow-md hover:shadow-lg';
    return `${baseStyles[status]} ${priorityGlow} transition-all duration-300`;
  };

  const getStatusColor = (status: KPIData['status'], isDark: boolean) => {
    const colors = {
      excellent: isDark ? 'text-green-400 bg-green-900/20 border-green-500/30' : 'text-green-600 bg-green-50 border-green-200',
      good: isDark ? 'text-blue-400 bg-blue-900/20 border-blue-500/30' : 'text-blue-600 bg-blue-50 border-blue-200',
      warning: isDark ? 'text-yellow-400 bg-yellow-900/20 border-yellow-500/30' : 'text-yellow-600 bg-yellow-50 border-yellow-200',
      critical: isDark ? 'text-red-400 bg-red-900/20 border-red-500/30' : 'text-red-600 bg-red-50 border-red-200'
    };
    ...
    <div className={`performance-kpi ${className}`}>
            ...
            <div className="flex items-center space-x-4 text-xs mt-1">
              ...
              {advancedMetrics && (
                <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                  advancedMetrics.riskLevel === 'low' ? 'bg-green-100 text-green-800' :
                  advancedMetrics.riskLevel === 'medium' ? 'bg-yellow-100 text-yellow-800' :
                  advancedMetrics.riskLevel === 'high' ? 'bg-orange-100 text-orange-800' :
      ...
      <div className={`grid gap-6 ${
        ...
        {kpiData.map((kpi, index) => (
          <div
            key={`${kpi.label}-${index}`}
            className={`relative border-2 rounded-xl p-6 cursor-pointer ${
              getAdvancedStatusStyle(kpi.status, theme === 'dark', kpi.priority)
            }`}
          ...
          >
            ...
            <div className="flex items-center justify-between text-xs">
              <div className="flex flex-col space-y-1">
                ...
                {kpi.changePercent !== undefined && Math.abs(kpi.changePercent) > 0.01 && (
            ...
            {kpi.label === 'Risk Level' && advancedMetrics && (
              <div className="mt-3 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{advancedMetrics.volatility.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between mt-1">
                  <span>Max DD:</span>
                  <span className="font-medium">{advancedMetrics.maxDrawdown.toFixed(1)}%</span>
                </div>
              </div>
            )}
          </div>
