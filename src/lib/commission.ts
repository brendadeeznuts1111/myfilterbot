...
export class CommissionCalculator {
  ...
  constructor() {
    // Safe initialization with fallbacks for undefined config objects
    ...
    this.performanceTiers = agentsConfig?.commission?.performance_tiers || [
      { min_volume: 0, rate_multiplier: 1.0 },
      { min_volume: 10000, rate_multiplier: 1.2 },
      { min_volume: 50000, rate_multiplier: 1.5 },
    ];

    // Log warnings if configs are missing
    if (!agentsConfig?.agents?.list) {
      console.warn(
        '⚠️ agents.yml config missing or invalid - using empty agent list'
      );
    }
    if (!agentsConfig?.masters?.list) {
      console.warn('⚠️ masters config missing - using empty master list');
    }
    if (!transactionsConfig?.transactions?.commission) {
      console.warn('⚠️ transaction commission config missing - using defaults');
    }
  }
  ...
  calculateAgentCommission(
    ...
    const winRate =
      settledBets.length > 0 ? wonBets.length / settledBets.length : 0;
  ...
  generateAgentStatement(agentId: string, month: string): any {
    ...
    const commission = this.calculateAgentCommission(
      agentId,
      month,
      sampleBets
    );
