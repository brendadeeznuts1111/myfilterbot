...
export class ApplicationBootstrap {
  ...
  async initialize(options: StartupOptions = {}): Promise<void> {
    ...
    try {
      ...
      console.log(`🎯 Ready to serve requests on port ${this.config?.server?.api?.port || 3000}`);
      console.log('=' + '='.repeat(59) + '\n');
  ...
  private async validateProductionConfig(): Promise<void> {
    ...
    const criticalChecks = [
      {
        condition: this.config.security.jwt.secret === 'dev-secret-key-change-in-production',
  ...
  private async runDatabaseMigrations(): Promise<void> {
    ...
    try {
      ...
      if (dbConfig.migrations?.autoRun) {
        console.log('   Running migrations from:', dbConfig.migrations.directory);
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    console.log(`     • ClickHouse: ${health.clickhouse ? '✅' : '⚠️  (optional)'}`);
