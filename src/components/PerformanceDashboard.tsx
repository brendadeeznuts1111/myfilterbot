...
}> = ({ title, value, change, changeType = 'neutral', loading, prefix = '', suffix = '', lastUpdated }) => {
  ...
  return (
    <div className="space-y-2 max-h-64 overflow-y-auto">
      ...
      {transactions.length > visibleTransactions && (
        ...
        >
          Show {Math.min(5, transactions.length - visibleTransactions)} more transactions
        </button>
      )}
    </div>
  );
...
export const PerformanceDashboard: React.FC<PerformanceDashboardProps> = ({ 
  ...
  const { data: balance, loading: balanceLoading, lastUpdated: balanceUpdated } = useCustomerBalance(customerId);
  const { data: analytics, loading: analyticsLoading, lastUpdated: analyticsUpdated } = useCustomerAnalytics(customerId);
  const { data: transactions, loading: transactionsLoading } = useTransactionHistory(customerId, 1, 10);
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-gray-600">
            Avg API Response: {performanceMetrics.averageResponseTime.toFixed(1)}ms
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard
          ...
          changeType={balance && balance.weekly_pnl > 0 ? 'positive' : balance && balance.weekly_pnl < 0 ? 'negative' : 'neutral'}
        />
        ...
        <MetricCard
          title="ROI %"
          value={analytics?.performance_metrics?.roi_percentage?.toFixed(2) || '0.00'}
          suffix="%"
          loading={analyticsLoading}
          lastUpdated={analyticsUpdated}
          changeType={analytics && analytics.performance_metrics.roi_percentage > 0 ? 'positive' : 
                      analytics && analytics.performance_metrics.roi_percentage < 0 ? 'negative' : 'neutral'}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">Balance Trend (30 Days)</h3>
            <div className="flex items-center space-x-2">
            ...
            <div className="mt-4 text-center">
              <p className="text-sm text-gray-600">
                Trend: <span className={`font-medium ${
                  ...
                  analytics.balance_trend[analytics.balance_trend.length - 1] > analytics.balance_trend[0]
          ...
          {analyticsLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center pt-2 border-t">
                <span className="text-sm font-medium text-gray-900">Net P&L</span>
                <span className={`font-bold ${
                  analytics.performance_metrics.net_pnl >= 0 ? 'text-green-600' : 'text-red-600'
                }`}>
                  {analytics.performance_metrics.net_pnl >= 0 ? '+' : ''}${analytics.performance_metrics.net_pnl.toLocaleString()}
                </span>
              </div>
            </div>
          ) : (
