...
const calculateTransactionVelocity = (transactions: TransactionData[]): number => {
  ...
  const timeSpan = new Date(recentTxns[0].timestamp).getTime() - 
...
const detectTransactionPatterns = (transactions: TransactionData[]): {
  ...
  const rapidFire = transactions.some((txn, index) => {
    ...
    const timeDiff = new Date(transactions[index - 1].timestamp).getTime() - 
                     new Date(txn.timestamp).getTime();
    return timeDiff < 60000; // Less than 1 minute apart
  });

  const roundNumbers = transactions.filter(txn => 
    txn.amount % 1000 === 0 || txn.amount % 500 === 0
  ).length > transactions.length * 0.7;

  const unusualTimes = transactions.filter(txn => {
    const hour = new Date(txn.timestamp).getHours();
    return hour < 6 || hour > 22; // Before 6 AM or after 10 PM
  }).length > transactions.length * 0.3;
...
const getTransactionRiskScore = (transaction: TransactionData): number => {
  ...
  if (transaction.description.includes('CASHAPP') || 
      transaction.description.includes('CHIME')) risk += 1;
...
export function RecentActivity({
  ...
  const { data: transactionData, error, loading, lastUpdated } = useAPI<{
    success: boolean;
    transactions: Array<{
      id: string;
      customer_id: string;
  ...
  const transactionAnalysis = useMemo(() => {
    ...
    const transactions = transactionData.transactions.map(tx => ({
      ...
      metadata: {
        method: tx.description.includes('CASHAPP') ? 'CashApp' : 
               tx.description.includes('CHIME') ? 'Chime' : 'Other',
        reference: tx.reference
      },
      tags: [
        tx.type === 'withdrawal' ? 'outgoing' : 'incoming',
        Math.abs(tx.amount) > largeTransactionThreshold ? 'large' : 'standard',
        tx.status === 'failed' ? 'failed' : tx.status === 'pending' ? 'pending' : 'completed'
      ]
    })) as TransactionData[];
    ...
    const largeTxns = transactions.filter(tx => Math.abs(tx.amount) > largeTransactionThreshold);
    ...
    return {
      ...
      insights: {
        ...
        averageAmount: transactions.reduce((sum, tx) => sum + Math.abs(tx.amount), 0) / transactions.length,
  ...
  const groupedTransactions = useMemo(() => {
    ...
    const grouped = transactionData.transactions.reduce((groups, transaction) => {
  ...
  const getTransactionTypeColor = (type: TransactionData['type']) => {
    const colors = {
      deposit: theme === 'dark' ? 'text-green-400 bg-green-900/20' : 'text-green-600 bg-green-50',
      withdrawal: theme === 'dark' ? 'text-red-400 bg-red-900/20' : 'text-red-600 bg-red-50',
      trade: theme === 'dark' ? 'text-blue-400 bg-blue-900/20' : 'text-blue-600 bg-blue-50',
      bonus: theme === 'dark' ? 'text-purple-400 bg-purple-900/20' : 'text-purple-600 bg-purple-50',
      fee: theme === 'dark' ? 'text-orange-400 bg-orange-900/20' : 'text-orange-600 bg-orange-50',
      adjustment: theme === 'dark' ? 'text-yellow-400 bg-yellow-900/20' : 'text-yellow-600 bg-yellow-50'
    };
    return colors[type] || (theme === 'dark' ? 'text-gray-400 bg-gray-900/20' : 'text-gray-600 bg-gray-50');
  };
  ...
  const getStatusColor = (status: TransactionData['status']) => {
    ...
    return colors[status] || (theme === 'dark' ? 'text-gray-400' : 'text-gray-600');
  };
  ...
  const isLargeTransaction = (amount: number) => {
    return highlightLarge && Math.abs(amount) >= largeTransactionThreshold;
  };
        ...
        {riskAnalysisEnabled && isHighRisk && (
          <div className="absolute top-2 right-2">
            <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse" 
                 title={`Risk Score: ${(riskScore * 100).toFixed(0)}%`}></div>
          </div>
        )}
        ...
        <div className="flex items-start justify-between mt-3">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              {riskAnalysisEnabled && riskScore > 0.5 && (
                <div className={`px-2 py-1 text-xs font-medium rounded-full ${
                  riskScore > 0.7 ? 'bg-red-100 text-red-800' :
                  riskScore > 0.5 ? 'bg-yellow-100 text-yellow-800' :
              ...
              <div className={`mt-4 p-3 rounded-lg ${
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-mono text-xs mt-1">{transaction.id}</div>
                  </div>
                  <div>
                    <span className="font-medium">Customer ID:</span>
                    <div className="font-mono text-xs mt-1">{transaction.customer_id}</div>
                  </div>
                  {riskAnalysisEnabled && (
                    <div className="col-span-2">
                      ...
                      <div className="mt-1 text-xs">
                        ...
                        Amount Tier: {Math.abs(transaction.amount) > 50000 ? 'Very High' : 
                                     Math.abs(transaction.amount) > 10000 ? 'High' : 
                                     Math.abs(transaction.amount) > 1000 ? 'Medium' : 'Low'}
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="text-right ml-6">
            ...
            <div className={`text-xs mt-1 ${
              ...
              {(() => {
                const timeDiff = Date.now() - new Date(transaction.timestamp).getTime();
          ...
          {transactionAnalysis && (
            <div className="flex items-center space-x-6 text-sm">
              <div className={`flex items-center space-x-1 ${
                ...
                <span>📊</span>
                <span>{transactionAnalysis.insights.totalTransactions} transactions</span>
              </div>
              ...
              {transactionAnalysis.insights.netFlow !== 0 && (
                <div className={`flex items-center space-x-1 ${
                  ...
                  <span>{transactionAnalysis.insights.netFlow > 0 ? '💰' : '📤'}</span>
                  <span>
              ...
              {transactionAnalysis.velocity > 0 && (
                <div className={`flex items-center space-x-1 ${
                  transactionAnalysis.velocity > 5 ? 'text-orange-600' : 'text-blue-600'
                }`}>
                  <span>⚡</span>
              ...
              {riskAnalysisEnabled && transactionAnalysis.insights.highRiskCount > 0 && (
                <div className="flex items-center space-x-1 text-red-600">
                  <span>⚠️</span>
                  <span>{transactionAnalysis.insights.highRiskCount} high risk</span>
                </div>
              )}
        ...
        {transactionAnalysis?.patterns.suspiciousPattern && (
          <div className={`p-4 rounded-lg border-l-4 ${
            ...
            <div className="flex items-center space-x-2">
              ...
              <div>
                <div className="font-semibold">Suspicious Transaction Pattern Detected</div>
                <div className="text-sm mt-1">
                  {transactionAnalysis.patterns.rapidFire && "• Rapid-fire transactions detected "}
                  {transactionAnalysis.patterns.roundNumbers && "• High frequency of round amounts "}
                  {transactionAnalysis.patterns.unusualTimes && "• Transactions at unusual hours"}
                </div>
              </div>
            </div>
          </div>
        ...
        {transactionAnalysis ? (
          groupByDate ? (
            Object.entries(groupedTransactions).map(([date, transactions]) => (
              <div key={date}>
                {date !== 'ungrouped' && (
                  <div className={`flex items-center justify-between mb-4 px-4 py-2 rounded-lg ${
            ...
            <div className="space-y-4">
              {transactionAnalysis.transactions.map(renderEnhancedTransactionItem)}
            </div>
          )
        ) : (
          <div className={`text-center py-12 ${
            ...
            <div className="text-sm">Try adjusting your filters or timeframe</div>
          </div>
        )}

        {/* Load more button with improved styling */}
        {transactionData?.pagination && transactionData.pagination.total_pages > 1 && (
          <div className="text-center pt-6">
            ...
            >
              📄 Load More Transactions ({transactionData.pagination.total - transactionData.transactions.length} remaining)
            </button>
          </div>
        )}
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              <div className={`text-2xl font-black ${
                transactionAnalysis.insights.netFlow > 0 ? 'text-green-500' : 'text-red-500'
              }`}>
            ...
            <div className="text-center">
              <div className={`text-2xl font-black ${
                transactionAnalysis.velocity > 5 ? 'text-orange-500' : 'text-blue-500'
              }`}>
            ...
            <div className="text-center">
              <div className={`text-2xl font-black ${
                transactionAnalysis.insights.highRiskCount > 0 ? 'text-red-500' : 'text-green-500'
              }`}>
